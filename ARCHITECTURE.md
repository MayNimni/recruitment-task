...
| `output.py` | B | 6–8 | referral selection, rationale, CSV writer, HTML writers (recruiter view + landing page) |
| `main.py` | both | — | argument parsing and sequencing, no logic |
| `llm.py` | B | 7 | the B7 seam's live implementation — opt-in, `match --llm` only |
| `build_aliases.py` | — | — | offline generator for `data/skill_aliases.json`, not imported by `main.py` |

**Dependency direction on the default path is one-way:** `main` → `{ingest, enrich, score, output}`.
`ingest`, `enrich` and `score` import no peer and no network library at all; `output` imports no peer
at module level. A default `ingest` / `match` / `run` therefore loads nothing that can open a socket.

**The two opt-in model paths are the only exceptions, and both are function-local imports** so that
the default path never executes them:
...
`llm.py` imports no peer of its own and holds no scoring logic — it is prompt construction, one API
call and response parsing. Nothing in `score.py` can reach it by any path.

The recruiter view and the landing page are both generated by `output.py`, not maintained by hand.
Each fills data literals into a template at the repo root — `recruiter_view.html` and
`index_template.html` — and neither generator touches the template's markup or CSS.

`write_index` is the one writer that **reads** from `output/`: the landing page spans every role
while `match` runs one at a time, so the committed `<id>_matches.csv` files are the only place all
four results exist at once. It is skipped on a `--data-dir` run, so a fixture can never rewrite the
landing page.
...
| Rationale sentence | B7 | template over component values, plus an opt-in live call | rationale builder, `pipeline/llm.py` |

**Default path: no model is called at runtime.** A3, A4 and B1 are deterministic in every run and
each carries a docstring naming the production implementation. Reason and trade-off:
`DECISIONS.md` §3.3.

**B7 is the one seam wired live**, behind `match --llm`. `pipeline/llm.py` is imported inside that
branch only (§7), so a default run cannot reach it and needs neither `ANTHROPIC_API_KEY` nor the
`anthropic` package.

With the flag set, a job costs at most **21 calls**: one per candidate for the top 20 ranked rows
(`apply_llm_briefs`, `shortlist_size=20`) writing `ai_summary` / `ai_probes`, and one writing the
shortlist summary line (`apply_llm_job_summary`). Both are **additive**. `build_match_row` has
already written the deterministic `rationale` / `interview_probes` and the recruiter view's note
`meaning`, and the model path never overwrites them; `call_candidate_brief` and
`call_shortlist_summary` return `None` on a malformed response or an API error, and the caller
treats `None` as an ordinary outcome — the field stays empty and the run finishes. The flag changes
what is *added* to a row, never what is *scored* in it, and never whether the run succeeds.

The summary line is the one field with no deterministic counterpart: it is model-or-nothing, so a
keyless run renders no line rather than a templated one.

Scoring is never a model call — with or without the flag.
