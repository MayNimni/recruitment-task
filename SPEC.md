...
requirements.txt  README.md  DECISIONS.md  SPEC.md
```

Python 3.10+, pandas only. No network calls, no API keys, no model calls at runtime.
...
`www.`. No name-based fallback — see `DECISIONS.md` §4.2.
...
seam described in `DECISIONS.md` §6.1:
...
returns structured signal tags, per `DECISIONS.md` §6.1.
...
example is named in `DECISIONS.md` §4.3 and must be a test case.
...
does not, and `DECISIONS.md` §4.4 already says so. `referral_feedback` is not a pool column: it is an
attribute of the candidate-employee pair, not of the candidate, so it lives only on
`pool/referral_edges.csv` — see `DECISIONS.md` §4.3 and the data model in `ARCHITECTURE.md` §6.
...
shared_employer, tier, referral_feedback`

`shared_employer` holds the matched company name or is empty. Never write "worked together" — the
candidate's past companies carry no dates.
...
Missing data must not read as poor fit — `DECISIONS.md` §4.2.
...
Carry `referral_why` as plain text: `both worked at Mobileye` for tiers A and C (both carry a
shared employer), `3 mutual connections` otherwise.
...
skills_matched, skills_semantic, skills_missing, rationale, interview_probes, referral_name,
...
because the adjustment sits on top of the fit score, never inside it — `DECISIONS.md` §4.3.
...
`index.html`, which is the unrelated assignment brief page. Copy it, then replace only the `DATA`
...
- No module imports a network library.
